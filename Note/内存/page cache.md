...
##### `vm.vfs_cache_pressure`
	...
	此参数控制内核回收目录和 inode 缓存的倾向，其取值范围是 0 到 200，默认值是 100。
...
#### `kswapd` 相关参数
	`kswapd` 是一个内核守护进程，负责内存回收

- **`vm.watermark_scale_factor`**：
    
    - 该参数调整[[#内存水印(watermark)]]（low, min, high）的比例，以决定何时启动 `kswapd`。
    - 较高的值会推迟 `kswapd` 启动，允许更多的页保持在 Active 状态。如果内存充足，这可以减少内存回收活动，提高性能。
    - 较低的值会加快 `kswapd` 启动，从而更频繁地回收内存页，这可能增加 Inactive 页的比例，因为 `kswapd` 会更积极地将不活跃的页移到 Inactive 列表。
      
- **`vm.min_free_kbytes`**：
    
    - 决定系统保持的最小空闲内存量。当可用内存量低于这个值时，内核会更积极地回收内存。
    - 设置较高的值可以确保系统维持较大的空闲内存量，减少 Inactive 和 Active 页被过早回收的风险。
...
#### 定义
	内存水印 (watermarks) 是 Linux 内核使用的一种机制，用于管理系统内存的分配和回收。内存水印有三个级别：`min`、`low` 和 `high`，它们分别定义了不同的内存压力阈值，以决定何时触发内核进行内存回收操作。理解这些水印的工作方式对于优化系统内存管理非常重要。

1. **min_watermark (`min`)**:
    
    - **最低水印**，系统在达到这个水印后，会强制性地回收内存，以确保有足够的空闲内存可用。
    - 一旦可用内存低于 `min` 水印，系统可能会出现性能下降，并且非关键进程可能会被强制挂起，直到足够的内存被释放。
      
2. **low_watermark (`low`)**:
    
    - **低水印**，当可用内存低于这个水印时，内核会激发更多的内存回收操作，但不会像 `min` 水印那样激进。
    - 此时，`kswapd` 守护进程会被唤醒并开始回收内存，以确保内存需求能够被满足。
      
3. **high_watermark (`high`)**:
    
    - **高水印**，这是理想状态下系统应保持的最低空闲内存量。
    - 当系统空闲内存高于 `high` 水印时，内核不会进行任何额外的回收操作，因为系统已处于健康状态。

#### 水印工作机制

当内核分配内存时，它会首先检查当前的可用内存量与这些水印值进行比较，并根据情况做出相应的内存回收操作。具体机制如下：

1. **超过 High 水印**：
    
    - 系统内存处于健康状态，不需要进行任何内存回收操作。
      
2. **介于 Low 和 High 水印之间**：
    
    - `kswapd` 将被唤醒并开始尝试回收内存，以维持空闲内存高于 `high` 水印。
    - 系统性能总体没有大幅度影响，但内核的内存回收线程将被激活以进行预防性回收。
      
3. **介于 Min 和 Low 水印之间**：
    
    - 内核将更加积极地进行内存回收。
    - 此时，`kswapd` 会持续运行以回收更多的内存，并确保系统稳定性。
      
4. **低于 Min 水印**：
    
    - 系统处于内存紧张状态，内存回收将变得非常激进。
    - 非关键进程可能会被挂起，`kswapd` 和直接内存回收都会被同时触发，以尽快释放内存。